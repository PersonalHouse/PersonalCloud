 ...
 * 
 * 1. Added explicit accessibility modifiers on properties and methods.
 * 2. Made '_authCb' readonly.
 */
...
namespace NSPersonalCloud.Apps.Album
{
    ...
    {
        private readonly Func<string, AlbumConfig> _authCb;
        ...
        private AlbumConfig AuthAndGetConfig()
        {
        ...
        private async Task GetFile(long id, bool asfile)
        {
            var cfg = AuthAndGetConfig();
            if (cfg == null)
            {
        ...
        // [Route(HttpVerbs.Get, "/list")] public async Task<List<ImageInfo>> List(int start, int
        // count) {
        //
        // using var dbConnection = new SQLiteConnection(Path.Combine(_albumFolder,
        // Defines.DBFileName), SQLiteOpenFlags.ReadOnly); return
        // dbConnection.Table<ImageInfo>().OrderByDescending(x => x.ExifDateTime)
